...
class Filemanager {
	...
	protected $logfile = 'C:\Wnmp\html\mbbox\log\filemanager.log';
	protected $cachefolder = '_thumbs/';
	protected $thumbnail_width = 64;
	protected $thumbnail_height = 64;
	protected $separator = ''; // @todo fix keep it or not?
	...
	public function __construct($extraConfig = '') {
		...
		$this->__log(__METHOD__ . " come in first if");
			} else if ($this->config['options']['s3ServerRoot'] === true) {
				$this->doc_root = $_SERVER['DOCUMENT_ROOT'];
				$_SESSION['email'] = '/muzammilpeer';
				$this->separator = $_SESSION['email'];
			$this->__log(__METHOD__ . " S3 Documents");
			} else{
				$this->doc_root = $this->config['options']['fileRoot'];
				$this->separator = basename($this->config['options']['fileRoot']);
		$this->__log(__METHOD__ . " come in first else");
			}
		} else {
		$this->__log(__METHOD__ . " come in second if");
	...
	public function getfolder() {
		...
		$this->__log(__METHOD__ . ' getFullPath ()  : ' . $this->getFullPath());
