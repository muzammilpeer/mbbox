...
var capabilities = config.options.capabilities || new Array('select', 'download', 'rename', 'move','share', 'delete');
...
function has_capability(data, cap) {
	if (data['File Type'] == 'dir' && cap == 'download' || data['File Type'] == 'dir' && cap == 'share') return false;
...
var getVideoPlayer = function(data) {
	var code  = '<video width=' + config.videos.videosPlayerWidth + ' height=' + config.videos.videosPlayerHeight + ' src="connectors/php/filemanager.php?mode=download&path=' + data['Path'] + '" controls="controls">';
...
var getAudioPlayer = function(data) {
	var code  = '<audio src="connectors/php/filemanager.php?mode=download&path=' + data['Path'] + '" controls="controls">';
		code += '<img src="' + data['Preview'] + '" />';
		code += '</audio>';
...
var bindToolbar = function(data){
	...
	if (!has_capability(data, 'share')) {
		$('#fileinfo').find('button#share').hide();
	} else {
		$('#fileinfo').find('button#share').click(function(){
			var newName = shareItem(data);
			if(newName.length) $('#fileinfo > h1').text(newName);
		}).show();
	}
...
// Share the current item to specified dir and returns the new name.
// Called by clicking the "Share" button in detail views
// or choosing the "Share" contextual menu option in
// list views.
var shareItem = function(data){
	var isShared = false;
	var msg = lg.confirmation_share;
	
	var doShare = function(v, m){
		if(v != 1) return false;	
		var connectString = fileConnector + '?mode=share&path=' + encodeURIComponent(data['Path']),
		...
		$.ajax({
			...
			success: function(result){
				if(result['Code'] == 0) {
					$.prompt('File Shared at :'+result['public-url']);
					/*
					...
					if(config.options.showConfirmation) $.prompt(lg.successful_share);
...
function getContextMenuOptions(elem) {
	...
	if (!($('#' + optionsID).length)) {
		...
		if (!elem.hasClass('cap_share')) $('.share', newOptions).remove();
...
var getFileInfo = function(file){
	...
	if($.inArray('share', capabilities)  != -1 && config.options.browseOnly != true) template += '<button id="share" name="share" type="button" value="Share">' + lg.share + '</button>';
...
$(function(){
	...
	if(config.options.autoload == true) {
		...
		$('#itemOptions a[href$="#share"]').append(lg.share);
